...
parser = argparse.ArgumentParser(description='Scraped stock to local.')
...
# this is the code to scrape news from NASDAQ on local and save the file to ./news_data/tickers.csv
# however we don't recommend to do so since NASDAQ will restrict the access very soon
def save_file(data,stock,mode = 'a+'):
    if not os.path.exists('./news_data/'):
        os.makedirs('./news_data/')
    print("saving {} news to file...".format(stock))
    if os.path.exists('./news_data/'+stock+'.csv'):
        header = False
    else:
        header = True
    data.to_csv('./news_data/'+stock+'.csv',mode = mode,index = False, encoding='utf_8_sig',header = header)
    return 
...
# this is the code to scrape news using NewYork Times API
"""
# for NewYork Times
API_KEY = ""

# https://api.nytimes.com/svc/search/v2/articlesearch.json?fq=romney&begin_date=20120101&end_date=20120101&api-key=your-api-key
response = requests.get("https://api.nytimes.com/svc/search/v2/articlesearch.json?q=tesla&fq=news_desk:(Business)&begin_date=20210101&end_date=20210201&api-key=")
soup = BeautifulSoup(response.text,'lxml')
data = json.loads(soup.p.text)
"""
...
# this is the code to simulate Chrome and scrape news from Yahoo! Finance
...
def get_info(url):
    ...
    #get imformation
    news_text = soup.find('div',attrs = {'class': 'caas-body'}).text
    news_title = soup.find('h1').text 
    news_date = datetime.strptime(soup.find('time').text,"%B %d, %Y, %H:%M %p")
    
    #combine all info into a list of columns
    columns = [news_text, news_title, news_date]
    
    #give columns names
    column_names = ['news','title','date']
...
def get_news(url,n):
    ...
    links = []
    batch = 10
    num_batches = n // batch + 1
    
    
    for number in tqdm(range(num_batches)):
    
        #get news url list from main page
        #links = links + ['https://finance.yahoo.com'+ \
        #                 driver.find_elements_by_css_selector('#Fin-Stream > ul > li:nth-child({}) a'.format(i))[0]['href'] \
        #                 for i in range(number*batch+1,(number+1)*batch+1)]
        elems = [driver.find_elements_by_css_selector('#Fin-Stream > ul > li:nth-child({}) a'.format(i))\
                 for i in range(number*batch+1,(number+1)*batch+1)]
        for e in elems:
            if len(e) == 1:
                links += [e[0].get_attribute('href')]
